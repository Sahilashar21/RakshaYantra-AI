...
//   // Cybercrime-only allowed keywords
//   const allowedKeywords = [
//     "cyber crime", "phishing", "malware", "ransomware", "hacking",
//     "data breach", "dark web", "identity theft",
//     "social engineering", "fraud", "ddos", "virus",
//     "trojan", "spyware", "botnet", "keylogger","cybercrime", "cyber attack", "cyber threat", "cybersecurity",
//   ];
...
//     const enforcedQuery = `${query} cybercrime`;
//     const URL = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${enforcedQuery}&type=video&maxResults=10&key=${API_KEY}`;
...
//   useEffect(() => {
//     fetchVideos("cybercrime tutorials");
//   }, []);
...
//     if (!isValid) {
//       alert("Please enter cybercrime-related searches only!");
//       return;
//     }
...
//           value={userInput}
//           onChange={(e) => setUserInput(e.target.value)}
//           placeholder="Search for cybercrime tutorials..."
//           style={{
//             padding: "12px",
//             width: "50%",
//             minWidth: "250px",
...
const Tutorials = () => {
  ...
  const allowedKeywords = [
    ...
    "data breach", "dark web", "identity theft",
    "social engineering", "fraud", "ddos", "virus",
    "trojan", "spyware", "botnet", "keylogger", "cybercrime",
    "cyber attack", "cyber threat","fishing"
  ];
  ...
  const fetchVideos = (query) => {
    ...
    axios
      ...
      .then((response) => {
        const videoItems = response.data.items.map((item) => ({
          id: item.id.videoId,
          title: decodeHTML(item.snippet.title),   // FIX APPLIED HERE
        }));
        setVideos(videoItems);
        setLoading(false);
      })
        ...
        {loading ? (
          ...
          videos.map((video) => (
            ...
            >
              ...
              >
                {decodeHTML(video.title)}   {/* FIX HERE */}
              </h3>
